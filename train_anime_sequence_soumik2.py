...
def dilate_boundary(image, mode=2):
    """ expects 0 value at boundary pixels, 255 at background pixels """
    k2 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2,2)).astype(np.float32)
    k3 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,3)).astype(np.float32)
    k4 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (4,4)).astype(np.float32)
    k5 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5)).astype(np.float32)
    k7 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7,7)).astype(np.float32)

    if mode == 1: # too thick
        kernel = torch.tensor(k3).to(image.device)
        # kernel = torch.tensor([[0, 0.5, 0],[0.5, 1, 0.5],[0, 0.5, 0]]).to(image.device) #didn't work
        dilated_image = morph.erosion(image, kernel)
    elif mode == 2: # a bit thick
        kernel = torch.tensor(k2).to(image.device)
        dilated_image = morph.erosion(image, kernel)
    elif mode == 3: # a bit thick (similar to 2)
        kernel1 = torch.tensor(k3).to(image.device)
        dilated_image = morph.erosion(image, kernel1)
        kernel2 = torch.tensor(k2).to(image.device)
        dilated_image = morph.dilation(dilated_image, kernel2)
    elif mode == 4: # ----> too dotty
        B, C, H, W = image.shape
        image = F.interpolate(image, (2*H, 2*W), mode='bilinear')
        kernel = torch.tensor(k3).to(image.device)
        dilated_image = morph.erosion(image, kernel)
        dilated_image = F.interpolate(dilated_image, (H, W), mode='bilinear')
    elif mode == 5: #---> correct width but lines very dotty
        B, C, H, W = image.shape
        image = F.interpolate(image, (2*H, 2*W), mode='bilinear')
        kernel = torch.tensor(k5).to(image.device)
        dilated_image = morph.erosion(image, kernel)
        dilated_image = F.interpolate(dilated_image, (H, W), mode='bilinear')
    elif mode == 6: #---> looks good I think
        B, C, H, W = image.shape
        image = F.interpolate(image, (2*H, 2*W), mode='bilinear',align_corners=False, antialias = True)
        kernel = torch.tensor(k5).to(image.device)
        dilated_image = morph.erosion(image, kernel)
        dilated_image = F.interpolate(dilated_image, (H, W), mode='bilinear', align_corners=False, antialias = True)
    elif mode == 7: #---> very blobby
        B, C, H, W = image.shape
        image = F.interpolate(image, (2*H, 2*W), mode='bilinear')
        kernel = torch.tensor(k7).to(image.device)
        dilated_image = morph.erosion(image, kernel)
        dilated_image = F.interpolate(dilated_image, (H, W), mode='bilinear')
    elif mode == 8: # fills small holes
        kernel1 = torch.tensor(k5).to(image.device)
        dilated_image = morph.erosion(image, kernel1)
        kernel2 = torch.tensor(k4).to(image.device)
        dilated_image = morph.dilation(dilated_image, kernel2)
    elif mode == 9: # correct thickness but fills holes sometimes
        kernel = torch.tensor(k3).to(image.device)
        dilated_image = morph.opening(image, kernel)
    elif mode == 10: # connected but too thin
        kernel = torch.tensor(k2).to(image.device)
        dilated_image = morph.opening(image, kernel)
    return dilated_image


def backwarp_sparse(flow, image, device='cuda'):
    """ image values should be in range [0,1], 
        0 corresponds to boundary pixels
        1 corresponds to background pixels"""
    W = image.shape[3]
    H = image.shape[2]
    gridX, gridY = np.meshgrid(np.arange(W), np.arange(H))
    gridX = torch.Tensor(gridX).unsqueeze(0).expand_as(flow[:,0,:,:]).float().to(device)
    gridY = torch.Tensor(gridY).unsqueeze(0).expand_as(flow[:,1,:,:]).float().to(device)
    ...
    imgout = F.grid_sample(image, grid, mode="nearest")
    # imgout = F.grid_sample(image, grid, mode='bilinear')

    # print(imgout.shape, mask.shape, grid.shape)
    return imgout
...
def training_loop(epoch, model, trainloader, trainset, optimizer, scheduler, \
    ...
    for trainIndex, trainData in enumerate(trainloader):
        ...
        if config.flow_type != None:
            _,_,_h, _w = masks[0].shape
            flow_up_13 = F.interpolate(flow[0],(_h,_w), mode='bilinear')
            flow_up_31 = F.interpolate(flow[1], (_h,_w), mode='bilinear')
        ...
        if config.gan_loss:
            loss += errG
        if config.warp_loss and epoch > config.warp_begin:
            if config.warp_sparse:
                img1_from_inb_orig = 1-backwarp_sparse(0.5*F12i, 1-outputs)
                img2_from_inb_orig = 1-backwarp_sparse(0.5*F21i, 1-outputs)  

                img1_from_inb = img1_from_inb_orig * srcmask
                img2_from_inb = img2_from_inb_orig * trgmask


                #additional background loss
                if config.warp_bg_loss:
                    img1_from_inb_bg = img1_from_inb_orig * (1-srcmask)
                    img2_from_inb_bg = img2_from_inb_orig * (1-trgmask)

                    img1_from_inb_sig_bg = torch.sigmoid(img1_from_inb_bg)
                    img2_from_inb_sig_bg = torch.sigmoid(img1_from_inb_bg)
                    loss_img1_bg = F.binary_cross_entropy(img1_from_inb_sig_bg, torch.zeros_like(img1_from_inb_sig_bg))
                    loss_img2_bg = F.binary_cross_entropy(img2_from_inb_sig_bg, torch.zeros_like(img2_from_inb_sig_bg))
                    loss += config.warp_weight*(loss_img1_bg + loss_img2_bg)
            ...
            if config.warp_dilate:
                img1_from_inb = 1-dilate_boundary((1-img1_from_inb)*255.)/255. #convert to black in bg, range[0,255], then convert back
                img2_from_inb = 1-dilate_boundary((1-img2_from_inb)*255.)/255. #convert to black in bg, range[0,255], then convert back
...
def testing_loop(epoch, model, testloader, testset, config, \
    ...
    with torch.no_grad():
        ...
        for validationIndex, validationData in enumerate(testloader):
            ...
            print("output_outfile", np.max(output_np[0]).item(), np.min(output_np[0]).item(), np.mean(output_np[0]).item())
            save_mask_to_img(output_np[0], output_outfile)
            save_mask_to_img(target_np[0], input_outfile)
            print("output_grayscale_outfile", np.max(output_not_binarized_np[0]).item(),np.min(output_not_binarized_np[0]).item(), np.mean(output_not_binarized_np[0]).item())
            save_mask_to_img(output_not_binarized_np[0], output_grayscale_outfile)
            ...
            if config.warp_loss and epoch > config.warp_begin:
                if config.warp_sparse:
                    ...
                    img2_from_inb = backwarp(0.5*F21i, outoutput_answerputs)
