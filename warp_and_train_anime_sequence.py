...
def backwarp_sparse(flow, image, device='cuda'):
    ...
    # remove black pixels which remaine because the flow was 0 at a black pixel in image
    # ie. remove boundary pixels which were not moved by flow 
    mask = torch.logical_or(flow[:,0:1,:,:]!=0, flow[:,1:2,:,:]!=0).float() #where there is non-zeroflow
    imgout = 1-(1-imgout)*mask 
...
def training_loop(epoch, model, trainloader, trainset, optimizer, scheduler, \
    ...
    backwarp_fn = lambda fl, x: 1-backwarp_sparse(fl,1-x)
    ...
    for trainIndex, trainData in enumerate(trainloader):
        ...
        #warp FIRST
        inb_from_src = backwarp_fn(0.5*F21i, srcmask)
        inb_from_trg = backwarp_fn(0.5*F12i, trgmask)
        inb_from_src = 1-dilate_boundary((1-inb_from_src)*255.)/255.
        inb_from_trg = 1-dilate_boundary((1-inb_from_trg)*255.)/255.
        ...
        if epoch % 10 == 0:
            print("saving first test warp")
            inb_from_src_outpath = os.path.join(config.train_store_path, "epoch_{:03d}".format(epoch), folder[1][0], "warp_"+index[1][0]+"_from_0.png")
            inb_from_trg_outpath = os.path.join(config.train_store_path, "epoch_{:03d}".format(epoch), folder[1][0], "warp_"+index[1][0]+"_from_2.png")
            inb_from_src_np = 255*(1 - inb_from_src[0].detach().cpu().numpy())
            inb_from_trg_np = 255*(1 - inb_from_trg[0].detach().cpu().numpy())
            cv2.imwrite(inb_from_src_outpath, np.transpose(inb_from_src_np, (1, 2, 0)))
            cv2.imwrite(inb_from_trg_outpath, np.transpose(inb_from_trg_np, (1, 2, 0)))
        ...
        input_cat = torch.cat([inb_from_src, inb_from_trg, srcmask, trgmask], 1)
...
def testing_loop(epoch, model, testloader, testset, config, \
    ...
    with torch.no_grad():
        ...
        for validationIndex, validationData in enumerate(testloader):
            ...
            if epoch % 10 == 0:
                print("saving first test warp")
                inb_from_src_outpath = os.path.join(config.test_store_path, "epoch_{:03d}".format(epoch), folder[1][0], "warp_"+index[1][0]+"_from_0.png")
                inb_from_trg_outpath = os.path.join(config.test_store_path, "epoch_{:03d}".format(epoch), folder[1][0], "warp_"+index[1][0]+"_from_2.png")
                inb_from_src_np = 255*(1 - inb_from_src[0].detach().cpu().numpy())
                inb_from_trg_np = 255*(1 - inb_from_trg[0].detach().cpu().numpy())
                cv2.imwrite(inb_from_src_outpath, np.transpose(inb_from_src_np, (1, 2, 0)))
                cv2.imwrite(inb_from_trg_outpath, np.transpose(inb_from_trg_np, (1, 2, 0)))
