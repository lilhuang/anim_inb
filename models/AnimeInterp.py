...
class AnimeInterp(nn.Module):
    ...
    def __init__(self, path='models/raft_model/models/rfr_sintel_latest.pth-no-zip', args=None):
        ...
        # self.initial = nn.Seqential(self.flownet, self.feat_ext, self.fwarp, self.synnet)

        #output from synnet is (batch x 3 x height x width)
        #want it to be (batch x 1 x height x width)
    ...
    def forward(self, I1, I2, F12i, F21i, t):
        ...
        #finetune for mask
        It_mask = self.final(It_warp)

        # return It_warp, F12, F21, F12in, F21in
        return It_mask, F12, F21, F12in, F21in
