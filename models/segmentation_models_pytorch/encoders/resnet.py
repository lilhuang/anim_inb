...
class ResNetEncoder(ResNet, EncoderMixin):
    def __init__(self, out_channels, depth=5, **kwargs):
        ...
        if depth >= 6:
            self.layer5 = self._make_layer(BasicBlock, 1024, 3, stride=2, dilate=False)
        if depth >= 7:
            self.layer6 = self._make_layer(BasicBlock, 2048, 3, stride=2, dilate=False)
    ...
    def get_stages(self):
        to_return = [
            ...
            self.layer4,]
        if self._depth >= 6:
            to_return.append(self.layer5)
        if self._depth >= 7:
            to_return.append(self.layer6)
        return to_return
...
resnet_encoders = {
    ...
    "resnet34_7": {
        ...
        "pretrained_settings": pretrained_settings["resnet34"],
        "params": {
            "out_channels": (3, 64, 64, 128, 256, 512, 1024, 2048),
            "block": BasicBlock,
            "layers": [3, 4, 6, 3],
            # "layers": [3, 4, 6, 4, 3],
        },
