...
def get_deformable_flow(flowObj, img1, mask_1, box_1, 
    ...
    if warp_func is not None:
        ...
        patch_flow_tensor = patch_flow_tensor.contiguous()
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--data_root', default="/vulcanscratch/lilhuang/Blender_test_large_output")
    # parser.add_argument('--csv_root', default="/vulcanscratch/lilhuang/ANCNet/datasets/blender_cube")
    parser.add_argument('--data_root', default="/vulcanscratch/lilhuang/DAIN/SU_24fps/SU_24fps/StevenHug")
    parser.add_argument('--csv_root', default="/vulcanscratch/lilhuang/202105_Share_ANCNet_WarpVisualization_Sample_Code_Models_ToyData/data_csv")
    parser.add_argument('--num_ib_frames', default=1)
    # parser.add_argument('--data_source', default="blender", help="blender | SU")
    parser.add_argument('--data_source', default="SU", help="blender | SU")
    # parser.add_argument('--output_root', default="/vulcanscratch/lilhuang/AnimeInterp/datasets/Blender_test_large_flows")
    parser.add_argument('--output_root', default="/vulcanscratch/lilhuang/AnimeInterp/datasets/SU_1ib_test_flows")
    ...
    image_names = [] #will have the shape #datapoints x 2 (index 0 source, index 1 target)
    csv_filename = args.data_source + "_" + str(args.num_ib_frames) + "ib.csv"
    csv_file = os.path.join(args.csv_root, csv_filename)
    with open(csv_file, newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
        for row in reader:
            src_name = os.path.join(args.data_root, row[0])
            trg_name = os.path.join(args.data_root, row[1])
            image_names.append([src_name, trg_name])
    ...
    for f_idx, imnames in enumerate(image_names):
        
        print('-- [%d/%d]'%(f_idx, end_idx-1))
        print(imnames)
        sys.stdout.flush()

        # img1 = cv2.imread(os.path.join(input_subfolder, imgFileNames[0]))
        # img3 = cv2.imread(os.path.join(input_subfolder, imgFileNames[-1]))
        img1 = cv2.imread(os.path.join(args.data_root, imnames[0]))
        img3 = cv2.imread(os.path.join(args.data_root, imnames[-1]))
        # pdb.set_trace()
        ...
        if label_root is None:
            ...
            colorMap1 = Image.fromarray(show_fill_map(labelMap1).astype("uint8"))
            colorMap3 = Image.fromarray(show_fill_map(labelMap3).astype("uint8"))

            ############## SAVE COLORMAPS!!!!! ############
            ################ actually...i think these are already saved in the flow dir ##########
        ...
        corrMap_np = corrMap.data.cpu().numpy()
        corrMap_img = Image.fromarray(corrMap_np, "L")
        ############# SAVE THIS IMAGE ####################
        ############# but also wtf does it mean????? ###############
        ############ because the match count is...94...out of 94???
        ...
        # save flows
        m0 = re.search(args.data_root+"/(.*).png",imnames[0])
        imname_first = m0.group(1)
        m1 = re.search(args.data_root+"/(.*).png",imnames[1])
        imname_second = m1.group(1)
        folder = imname_first + "_to_" + imname_second
