    ...
    for trainIndex, trainData in enumerate(trainloader):
        ...
        if config.warp_loss and epoch > config.warp_begin:
            if config.warp_sparse:
                backwarp_fn = lambda fl, x : 1-backwarp_sparse(fl,1-x) #convert to black in fg
            else:
                backwarp_fn = backwarp
            
            img1_from_inb = backwarp_fn(0.5*F12i, outputs)
            img2_from_inb = backwarp_fn(0.5*F21i, outputs)
...
def testing_loop(epoch, model, testloader, testset, config, \
    ...
    with torch.no_grad():
        ...
        for validationIndex, validationData in enumerate(testloader):
            ...
            if config.warp_loss and epoch > config.warp_begin:
                if config.warp_sparse:
                    backwarp_fn = lambda fl, x : 1-backwarp_sparse(fl,1-x) #convert to black in fg and then convert back
                else:
                    backwarp_fn = backwarp
                
                img1_from_inb = backwarp_fn(0.5*F12i, output_answer)
                img2_from_inb = backwarp_fn(0.5*F21i, output_answer)
...
def main(config, args):
    ...
    if config.dataset == "atd12k":
        ...
        print("config.flow_type", config.flow_type)
