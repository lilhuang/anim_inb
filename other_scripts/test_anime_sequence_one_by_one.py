...
def validate(config):  
    ...
    testset = datas.AniTripletWithSGMFlowTest(config.testset_root, config.test_flow_root, \
                                              trans, config.test_size, config.test_crop_size, \
                                              train=False)
    # pdb.set_trace()
    ...
    with torch.no_grad():
        ...
        for validationIndex, validationData in enumerate(validationloader, 0):
            ...
            ITs = [sample[tt] for tt in range(0, 1)]
            I1 = frame1.cuda()
            I2 = frame2.cuda()
